# AttachAppPolicyToIdentity {#doc_api_vod_AttachAppPolicyToIdentity .reference}
...
|Action|String|是|AttachAppPolicyToIdentity|系统规定参数，取值：**AttachAppPolicyToIdentity**。
 ...
 |
|AppId|String|是|app-xxxxx|应用ID。

 **说明：** 当策略名称为VODAppAdministratorAccess时，该字段非必选，其他策略时，该字段必填。

 |
|IdentityName|String|是|xxxx|-   类型为RamUser时，传入RAM子账号ID。
-   类型为RamRole时，传入角色名称。

 |
|IdentityType|String|是|RamRole|身份类型。取值：**RamUser**（RAM子账号）, **RamRole**（RAM角色）。

 |
|PolicyNames|String|是|VODAppFullAccess|策略名称。多个用英文逗号分隔。
...
|名称|类型|示例值|描述|
|--|--|---|--|
|FailedPolicyNames| |VODAppFullAccess|失败的策略名称。

 |
|NonExistPolicyNames| |VODAppFullAccess|不存在的策略名称列表。
...
http(s)://[Endpoint]/?Action=AttachAppPolicyToIdentity
&AppId=app-xxxxx
&IdentityName=xxxx
