...
支持对已接入点播服务的OSS bucket中已存在的音视频媒体文件的注册，注册媒资成功后，可根据媒体ID发起[转码](~~68570~~)、[截图](~~72213~~)和AI处理（如[审核](~~89869~~)）等。
...
|RegisterMetadatas|String|是|\{"Title":"ceshi"\}|注册媒资元数据。为JSON字符串，一次最多支持设置10个媒资元数据。参数结构详情，请参见[MediaMetadata](~~52839~~)。 |
