...
添加点播加速域名。每次只能提交一个加速域名，一个用户最多添加20个域名。

**说明：**

-   创建加速域名之前，您需要先开通[点播服务](~~51512~~)。
-   加速域名必须已备案完成。
...
|Sources|String|是|\[\{"content":"1.1.1.1","type":"ipaddr","priority":"20","port":80\}\]|回源地址列表。参数详情参见下表Sources描述。 |
|CheckUrl|String|否|www.example.com/test.html|检查域名是否能正常访问url。 |
|Scope|String|否|domestic|国际用户、国内L3及以上用户设置有效。
