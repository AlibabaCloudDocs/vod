...
1.  创建播放器，示例如下：
    ...
    如果有配置安全下载功能（即经过阿里云加密转码过后的），那么还需要设置一个加密校验信息，传入encryptedApp.dat文件的内容，请参见[安全文件获取问题](/cn.zh-CN/常见问题/播放器/安全文件获取.md)。
...
15. 配置网络重试时间和次数
    ...
    **说明：**
    ...
    -   如果重试多次之后，还是loading的状态，那么就会回调onError事件，此时AVPErrorModel.code为ERROR\_LOADING\_TIMEOUT。
