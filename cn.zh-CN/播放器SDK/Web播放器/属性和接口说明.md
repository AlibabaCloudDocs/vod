...
**说明：** source支持多清晰度设置：source:’\{“HD”:”address1”,”SD”:”address2”\}’，详情请参见[多清晰度播放](/cn.zh-CN/播放器SDK/Web播放器/功能使用说明.md)。 |
...
**说明：** chrome浏览器下flash播放器分别不能小于397x297px。 |
|videoWidth|String|视频宽度，仅HTML5支持。详情请参见[旋转和镜像](/cn.zh-CN/播放器SDK/Web播放器/功能使用说明.md)。|
|videoHeight|String|视频高度，仅H5支持。详情请参见[旋转和镜像](/cn.zh-CN/播放器SDK/Web播放器/功能使用说明.md)。|
...
|controlBarVisibility|String|控制面板的实现，默认为值为：**hover**。取值：-   **click**：点击
-   **hover**：停留
-   **always** |
|showBarTime|String|控制栏自动隐藏时间（ms）。|
|extraInfo|String|JSON串用于定制性接口参数，目前支持：-   fullTitle：测试页面，全屏时显示视频标题（仅flash支持）
-   m3u8BufferLength：播放m3u8时加载缓存ts文件长度单位（秒）（仅flash支持）
-   liveStartTime：2016/08/17 12:00:00，直播开始时间，用于提示直播未开始\(仅flash支持\)
-   liveOverTime：2016/08/17 14:00:00，直播结束时间，用于提示直播结束（仅flash支持） |
...
|x5\_fullscreen|Boolean|声明视频播放时是否进入到TBS的全屏模式，默认值为**false**。取值：-   **false**
-   **true**：把视频做为背景
...
|setRotate|rotate 旋转角度|参数为旋转角度， 正数为正时针旋转， 负数为逆时针旋转。示例：setRotate\(90\)。详情请参见[旋转和镜像](/cn.zh-CN/播放器SDK/Web播放器/功能使用说明.md)。|
|getRotate|无|获取旋转角度。详情请参见[旋转和镜像](/cn.zh-CN/播放器SDK/Web播放器/功能使用说明.md)。|
|setImage|image：镜像类型|设置镜像，取值：-   **horizon**
-   **vertical**

示例：setImage\(‘horizon’\)。详情请参见[旋转和镜像](/cn.zh-CN/播放器SDK/Web播放器/功能使用说明.md)。|
|dispose|无|播放器销毁|
