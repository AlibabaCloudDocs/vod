...
5.  配置RAM角色。关于RAM角色，请参见[RAM子账号](/intl.zh-CN/开发指南/账号和授权/概述.md)。
    ...
    9.  将RAM角色附加到ECS实例上。
        ...
        在OpenAPI开发者门户使用ECS的[AttachInstanceRamRole](/intl.zh-CN/API参考/实例/AttachInstanceRamRole.md)接口，详情请参见[OpenAPI开发者门户](https://next.api.aliyun.com/api/Ecs/2014-05-26/AttachInstanceRamRole)。
    ...
    10. 在ECS终端执行以下命，验证RAM角色和ECS实例是否关联成功。
        ...
        **说明：** 每个ECS实例只可附加一个RAM角色，如果您需要更换RAM角色，需要在[OpenAPI开发者门户](https://next.api.aliyun.com/api/Ecs/2014-05-26/AttachInstanceRamRole?tab=DEBUG)中调用[DetachInstanceRamRole](/intl.zh-CN/API参考/实例/DetachInstanceRamRole.md)，解除该ECS上的RAM角色附加后，再次使用[AttachInstanceRamRole](/intl.zh-CN/API参考/实例/AttachInstanceRamRole.md)重新附加RAM角色。
