...
调用CreateUploadVideo接口，生成上传凭证、上传地址和VideoID的时候，点播控制台视频列表会有一个状态为"上传中"的"占位"，如果客户端后续不进行实际视频上传，或者上传报错，或者是正在上传的时候把上传进程终断了（比如结束APP进程），都会导致控制台这个视频状态一直为"上传中"，请调用[RefreshUploadVideo（刷新视频上传凭证）](/intl.zh-CN/服务端API/媒体上传/刷新视频上传凭证.md)接口，传入这个视频ID去获取新的凭证和地址，再去上传，或者手动删除掉这个状态为上传中的视频。视频实际上传，请调用[UploadMediaByURL（URL批量拉取上传）](/intl.zh-CN/服务端API/媒体上传/URL批量拉取上传.md)接口。
